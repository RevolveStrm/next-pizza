...
export const CheckboxFiltersGroup: React.FC<Props> = ({
    ...
    const shownCheckboxes = showAll ?
        checkboxes : checkboxes.slice(0, limit);

    const filteredCheckboxes = searchInputValue ?
        shownCheckboxes.filter(el => el.text.toLowerCase().includes(searchInputValue.toLowerCase())) : shownCheckboxes;
    ...
    if (loading) {
        return <div className={className}>
            ...
            {...Array(limit)
                .fill(0)
                .map((el, idx) => (<Skeleton key={idx} className='h-6 mb-5 rounded-[8px]' />))}
    ...
    return (
        <div className={className}>
            ...
            {showAll &&
                <div className='mb-5'>
                    <Input onChange={handleChangeInputValue} placeholder={searchInputPlaceholder} className='bg-gray-50 border-none' />
                </div>
            }
