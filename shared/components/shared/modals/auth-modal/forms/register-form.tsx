...
import { RegistrationFormFields, registrationFormSchema } from 'shared/constants/auth-form-schema';
...
export const RegisterForm: React.FC<Props> = ({ className, onClose, onSuccess }) => {
        ...
        <FormProvider {...form}>
            <form className='flex flex-col gap-5' onSubmit={form.handleSubmit(onSubmit)}>
                <div className='flex justify-between items-center'>
                    <div className='mr-2'>
                        <Title text='Реєстрація акаунту' size='md' className='font-bold' />
                        <p className='text-gray-400'>Введіть свої, щоб зареєструвати акаунт</p>
                    </div>
                </div>
                ...
                <Button disabled={form.formState.isSubmitting} className='h-12 text-base' type='submit'>
                    {
                        form.formState.isSubmitting ? 'Реєструємо...' : 'Зареєструватись'
                    }
                </Button>
            </form>
        </FormProvider>
